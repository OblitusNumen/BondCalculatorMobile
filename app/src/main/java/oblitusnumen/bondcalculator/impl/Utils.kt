...
fun getBonds(context: Context): List<Bond> =
    getSharedPrefs(context).getStringSet(BONDS_SHARED_PREFS, emptySet<String>())!!
        .map { bond -> Bond.fromString(bond)!! }

fun putBonds(context: Context, bonds: Collection<Bond>) =
...
fun getBond(context: Context, bondId: Int?): Bond? =
    getBonds(context).firstOrNull { it.id == bondId }

fun saveBond(context: Context, bond: Bond) =
    putBonds(context, getBonds(context).toMutableSet().apply {
        removeIf { it.id == bond.id }
        add(bond)
    })

fun deleteBond(context: Context, bond: Bond) =
    putBonds(context, getBonds(context).toMutableSet().apply {
        removeIf { it.id == bond.id }
    })
